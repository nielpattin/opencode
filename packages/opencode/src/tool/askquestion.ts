...
export const AskQuestionTool = Tool.define("askquestion", {
  ...
  parameters: z.object({
    questions: z.array(AskQuestion.QuestionSchema).min(3).max(5).describe("3-5 questions to ask in wizard flow"),
  }),
  ...
  async execute(params, ctx) {
    ...
    const answerPromise = AskQuestion.register(ctx.callID!, ctx.sessionID, ctx.messageID, params.questions)
